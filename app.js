...
function addTask() {
  ...
  if (textareaTask.value === "") {
    ...
    let task = textareaTask.value
    listaStorage.push(task)
    localStorage.setItem("listaStorage", JSON.stringify(listaStorage))
    createLiTaskElement(task)
    modal.classList.remove("mostrarModal")  
  }
      textareaTask.value=""
}

function createLiTaskElement(task){
  let li = document.createElement("li");
    ...
    buttonTrash.addEventListener("click", function () {
      ...
      listaStorage = listaStorage.filter((t)=> t !== task)
      localStorage.setItem("listaStorage", JSON.stringify(listaStorage))
    });
...
function loadTasks() {
  listaStorage.forEach((task) => createLiTaskElement(task));
}
